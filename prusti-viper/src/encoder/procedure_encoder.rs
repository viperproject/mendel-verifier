...
impl<'p, 'v: 'p, 'tcx: 'v> ProcedureEncoder<'p, 'v, 'tcx> {
    ...
    fn encode_sequence_index_call(
        ...
        trace!(
            "encode_sequence_index_call(destination={:?}, args={:?})",
            destination,
            args
        );
    ...
    fn encode_impure_function_call(
        ...
        if self.encoder.env().query.is_closure(called_def_id) {
            ...
            substs = self
                ...
                .env()
                .tcx()
                .mk_substs(self.substs.iter().chain(substs));
        } else {
            ...
            match encoded_operand {
                Some(place) => {
                    ...
                    if !self.encoder.is_pure(called_def_id, Some(substs)) {
                        let opt_inv = self
                            .encoder
                            .encode_invariant_func_app(arg_ty, vir::Expr::snap_app(place.clone()))
                            .with_span(call_site_span)?;
    ...
    fn encode_assign_aggregate(
        ...
        match *aggregate {
            ...
            mir::AggregateKind::Closure(def_id, substs) => {
                ...
                assert!(
                    !self.encoder.is_spec_closure(def_id.to_def_id()),
                    "spec closure: {def_id:?}"
                );
