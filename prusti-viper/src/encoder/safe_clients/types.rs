...
pub fn encode_type_name<'v, 'tcx: 'v>(
    ...
    Ok(match ty.kind() {
        ...
        &ty::TyKind::FnPtr(sig) => {
            ...
            format!(
                ...
                sig.inputs()
                    .iter()
                    .map(|&input_ty| encode_type_name(encoder, input_ty))
...
fn encode_type_argument_name<'v, 'tcx: 'v>(
    encoder: &Encoder<'v, 'tcx>,
    arg: ty::subst::GenericArg<'tcx>,
) -> EncodingResult<String> {
    match arg.unpack() {
...
pub fn snapshot_might_contain_references<'tcx>(tcx: ty::TyCtxt<'tcx>, ty: ty::Ty<'tcx>) -> bool {
    ...
    match *ty.kind() {
        ...
        ty::TyKind::Array(inner_ty, _) | ty::TyKind::Slice(inner_ty) => {
            snapshot_might_contain_references(tcx, inner_ty)
        }
        ty::TyKind::Tuple(elem_tys) => elem_tys.iter().any(inner_visitor),
...
pub fn is_opaque_type<'tcx>(tcx: ty::TyCtxt<'tcx>, ty: ty::Ty<'tcx>) -> bool {
    ...
    if type_name == "std::boxed::Box"
        || type_name == "std::ptr::Unique"
        || type_name == "std::ptr::NonNull"
    {
        return false;
    }

    // Small optimization: types with no public fields cannot be described using #[extern_spec]
    let has_public_fields = adt_def
        .variants()
        .iter()
        .any(|v| v.fields.iter().any(|f| f.vis.is_public()));
