...
impl<'p, 'v: 'p, 'tcx: 'v> VersionBasedProcedureEncoder<'p, 'v, 'tcx> {
    ...
    pub(super) fn encode_rvalue_snapshot(
        &self, rhs: &mir::Rvalue<'tcx>, version_kind: Version,
    ) -> EncodingResult<vir::Expr> {
        let expr = self.fixed_version_encoder(version_kind)
    ...
    pub(super) fn encode_operand_snapshot(
        &self, operand: &mir::Operand<'tcx>, version_kind: Version,
    ) -> EncodingResult<vir::Expr> {
    ...
    pub(super) fn encode_place_address(
        &self, place: mir::Place<'tcx>, version_kind: Version,
    ) -> EncodingResult<vir::Expr> {
        let opt_result = self.fixed_version_encoder(version_kind).encode_place_address(place)?;
    ...
    pub(super) fn encode_place_snapshot(
        ...
        let expr = self.fixed_version_encoder(version_kind).encode_place_snapshot(place)?;
        if !expr.kind().is_memory() {
            error_internal!("cannot determine the address of place '{place:?}'")
        }
        Ok(expr.into_expr())
