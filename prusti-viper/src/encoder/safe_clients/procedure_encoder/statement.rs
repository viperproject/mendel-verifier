...
use analysis::mir_utils::{place_ref_to_place, remove_place_from_set, Place, PlaceImpl};
        ...
        let encoding_stmts = match stmt.kind {
            ...
            mir::StatementKind::Assign(box (lhs, ref rhs)) => {
                ...
                let rhs_snapshot = self
                    .encode_rvalue_snapshot(rhs, Version::Old)
                    .with_span(span)?;
                let lhs_snapshot = self
                    .encode_place_snapshot(lhs, Version::CurrOld)
                    .with_span(span)?;
                trace!("Encoding of {lhs:?} = {rhs:?} is: {lhs_snapshot} == {rhs_snapshot}");
                let mut assign_stmts = vec![vir::Stmt::inhale(
                    vir_expr! { [lhs_snapshot] == [rhs_snapshot] },
                )];
                ...
                if lhs.projection.last() == Some(&mir::ProjectionElem::Deref) {
                    let lhs_ref = mir::Place {
                        ...
                        projection: self
                            .tcx()
                            .intern_place_elems(&lhs.projection[..lhs.projection.len() - 1]),
                    };
                    let lhs_ref_ty = self.get_place_ty(lhs_ref).ty;
                    let old_lhs_ref_snap = self
                        .encode_place_snapshot(lhs_ref, Version::Old)
                        .with_span(span)?;
                    let curr_lhs_ref_snap = self
                        .encode_place_snapshot(lhs_ref, Version::CurrOld)
                        .with_span(span)?;
                    let old_lhs_target = self
                        .encode_snapshot_domain(SnapshotKind::Memory, lhs_ref_ty)
                        .with_span(span)?
                        .target_address_function()
                        .with_span(span)?
                        .apply1(old_lhs_ref_snap);
                    let curr_lhs_target = self
                        .encode_snapshot_domain(SnapshotKind::Memory, lhs_ref_ty)
                        .with_span(span)?
                        .target_address_function()
                        .with_span(span)?
                        .apply1(curr_lhs_ref_snap);
                ...
                if let &mir::Rvalue::Use(mir::Operand::Move(rhs_place)) = rhs {
                    ...
                    let old_rhs_address = self
                        .encode_place_address(rhs_place, Version::Old)
                        .with_span(span)?;
                    let curr_lhs_address = self
                        .encode_place_address(lhs, Version::CurrOld)
                        .with_span(span)?;
                    let old_version = self.encode_version(Version::Old);
                    let curr_version = self.encode_version(Version::CurrOld);
                    let ownership_domain =
                        OwnershipDomain::encode(self.encoder, lhs_ty).with_span(span)?;
                    let move_fact = ownership_domain
                        .moved_fact_function()
                        .with_span(span)?
                        .apply4(old_rhs_address, old_version, curr_lhs_address, curr_version);
                    ...
                    for (index, &framed_place) in owned_places.iter().enumerate() {
                        ...
                        let place_address = self
                            .encode_place_address(*framed_place, Version::Old)
                            .with_span(span)?;
                        let old_version = self.encode_version(Version::Old);
                        let curr_version = self.encode_version(Version::CurrOld);
                        let place_ty = self.get_place_ty(*framed_place).ty;
                        let ownership_domain =
                            OwnershipDomain::encode(self.encoder, place_ty).with_span(span)?;
                        ...
                        assign_stmts.extend(vec![
                            vir::Stmt::comment(format!(
                                "Fully-owned used place {framed_place:?} does not change"
                            )),
                ...
                if let mir::Rvalue::Aggregate(box kind, operands) = rhs {
                    let variant_idx =
                        if let &mir::AggregateKind::Adt(_, variant_idx, _, _, _) = kind {
                            Some(variant_idx)
                        } else {
                            None
                        };
                    for (field_idx, operand) in operands.iter().enumerate() {
                        if let &mir::Operand::Move(place) = operand {
                            let old_place_address = self
                                .encode_place_address(place, Version::Old)
                                .with_span(span)?;
                            let curr_lhs_address = self
                                .encode_place_address(lhs, Version::CurrOld)
                                .with_span(span)?;
                            let lhs_address_domain =
                                AddressDomain::encode(self.encoder, lhs_ty).with_span(span)?;
                            let curr_place_address = lhs_address_domain
                                .adt_field_address_function(variant_idx, field_idx.into())
                                .with_span(span)?
                                .apply1(curr_lhs_address);
                            let old_version = self.encode_version(Version::Old);
                            let curr_version = self.encode_version(Version::CurrOld);
...
fn get_dereferenced_mut_ref<'tcx>(
    body: &mir::Body<'tcx>,
    tcx: ty::TyCtxt<'tcx>,
    place: mir::Place<'tcx>,
) -> Option<Place<'tcx>> {
