...
use analysis::{
    ...
    domains::{
        DefinitelyAccessibleAnalysis, DefinitelyAccessibleState, DefinitelyAllocatedAnalysis,
        DefinitelyAllocatedState, DefinitelyInitializedAnalysis, DefinitelyUnreachableAnalysis,
        DefinitelyUnreachableState, FramingAnalysis, FramingState, LocallySharedAnalysis,
        LocallySharedState,
    },
    mir_utils::Place,
    PointwiseState,
...
impl<'p, 'v: 'p, 'tcx: 'v> VersionBasedProcedureEncoder<'p, 'v, 'tcx> {
    ...
    pub(super) fn compute_facts(
        ...
        // Load facts
        let borrowck_facts = self
            .encoder
            .env()
            .body
            .local_mir_borrowck_facts(local_def_id);
        ...
        let ownership_facts =
            match ownership_analysis.run_analysis(input_facts, &output_facts, location_table) {
        ...
        let locally_shared_analysis =
            LocallySharedAnalysis::new(tcx, self.def_id, self.mir, ownership_facts.clone());
        ...
        let framing_facts =
            match framing_analysis.run_analysis(input_facts, &output_facts, location_table) {
                Ok(state) => state,
                Err(err) => {
                    return Err(SpannedEncodingError::internal(
        ...
        let def_initialized_analysis =
            DefinitelyInitializedAnalysis::new(tcx, self.def_id, self.mir);
        ...
        let def_unreachable_analysis = match DefinitelyUnreachableAnalysis::new(
            ...
            Err(err) => {
                return Err(SpannedEncodingError::internal(
                    format!(
                        "error while constructing the definitely-unreachable analysis: {err:?}"
                    ),
                    self.mir.span,
                ));
            }
    ...
    pub(super) fn encode_framing_at_precondition(&self) -> EncodingResult<Vec<vir::Stmt>> {
        ...
        for &place in facts.get_definitely_owned() {
            ...
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume framing of WriteRef({place:?}: {:?})",
                    place.ty(self.mir, self.tcx()).ty
                )),
        ...
        for &place in &locally_shared_facts.get_locally_shared_only(facts) {
            ...
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume framing of LocalRef({place:?}: {:?})",
                    self.get_local_ty(place)
                )),
                vir::Stmt::inhale(fact),
            ]);
        }
        for &place in &locally_shared_facts.get_definitely_accessible_only(facts) {
            ...
            let fact = self.encode_framed_ownership_fact(
                ...
                fact_idx,
                place.into(),
                OwnershipKind::ReadRef,
            )?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume framing of ReadRef({place:?}: {:?})",
                    place.ty(self.mir, self.tcx()).ty
                )),
                vir::Stmt::inhale(fact),
            ]);
    ...
    pub(super) fn encode_framing_at_postcondition(&self) -> EncodingResult<Vec<vir::Stmt>> {
        ...
        for &place in &deeply_unreachable {
            let fact = self.encode_framed_ownership_fact(
                ...
                fact_idx,
                place.into(),
                OwnershipKind::DeeplyUnreachable,
            )?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume framing of DeeplyUnreachable({place:?}: {:?})",
                    place.ty(self.mir, self.tcx()).ty
                )),
                vir::Stmt::inhale(fact),
            ]);
        ...
        for &place in &shallowly_unreachable {
            ...
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume framing of ShallowlyUnreachable({place:?}: {:?})",
                    place.ty(self.mir, self.tcx()).ty
                )),
                vir::Stmt::inhale(fact),
            ]);
    ...
    /// The framing is between the old and the current memory version.
    pub(super) fn encode_framing_across_location(
        &self,
    ...
    /// The framing is between the old and the current memory version.
    pub(super) fn encode_framing_before_location(
        &self,
        location: mir::Location,
    ) -> SpannedEncodingResult<Vec<vir::Stmt>> {
    ...
    pub(super) fn encode_ownership_at_precondition(&self) -> EncodingResult<Vec<vir::Stmt>> {
        ...
        for &place in facts.get_definitely_owned() {
            fact_idx += 1;
            let fact =
                self.encode_ownership_fact(fact_idx, place.into(), OwnershipKind::WriteRef)?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume WriteRef({place:?}: {:?})",
                    place.ty(self.mir, self.tcx()).ty
                )),
                vir::Stmt::inhale(fact),
            ]);
        ...
        for &place in &locally_shared_facts.get_locally_shared_only(facts) {
            fact_idx += 1;
            let fact =
                self.encode_ownership_fact(fact_idx, place.into(), OwnershipKind::LocalRef)?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume LocalRef({place:?}: {:?})",
                    self.get_local_ty(place)
                )),
                vir::Stmt::inhale(fact),
            ]);
        ...
        for &place in &locally_shared_facts.get_definitely_accessible_only(facts) {
            fact_idx += 1;
            let fact =
                self.encode_ownership_fact(fact_idx, place.into(), OwnershipKind::ReadRef)?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume ReadRef({place:?}: {:?})",
                    place.ty(self.mir, self.tcx()).ty
                )),
                vir::Stmt::inhale(fact),
            ]);
        ...
        for &place in allocation_facts.get_def_allocated_locals() {
            fact_idx += 1; // Dummy
            let fact =
                self.encode_ownership_fact(fact_idx, place.into(), OwnershipKind::Allocated)?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume Allocated({place:?}: {:?})",
                    self.get_local_ty(place)
                )),
                vir::Stmt::inhale(fact),
            ]);
    ...
    /// All derefs will use the current memory version.
    pub(super) fn encode_ownership_before_location(
        &self,
        location: mir::Location,
    ) -> SpannedEncodingResult<Vec<vir::Stmt>> {
        ...
        for &place in facts.get_definitely_owned() {
            fact_idx += 1;
            let fact = self
                .encode_ownership_fact(fact_idx, place.into(), OwnershipKind::WriteRef)
                .with_span(span)?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume WriteRef({place:?}: {:?})",
        ...
        for &place in &locally_shared_facts.get_locally_shared_only(facts) {
            fact_idx += 1;
            let fact = self
                .encode_ownership_fact(fact_idx, place.into(), OwnershipKind::LocalRef)
                .with_span(span)?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume LocalRef({place:?}: {:?})",
        ...
        for &place in &locally_shared_facts.get_definitely_accessible_only(facts) {
            fact_idx += 1;
            let fact = self
                .encode_ownership_fact(fact_idx, place.into(), OwnershipKind::ReadRef)
                .with_span(span)?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume ReadRef({place:?}: {:?})",
        ...
        for &place in allocation_facts.get_def_allocated_locals() {
            fact_idx += 1; // Dummy
            let fact = self
                .encode_ownership_fact(fact_idx, place.into(), OwnershipKind::Allocated)
                .with_span(span)?;
            stmts.extend(vec![
                vir::Stmt::comment(format!(
                    "assume Allocated({place:?}: {:?})",
    ...
    /// Encode ownership before a location.
    /// All derefs will use the current memory version.
    pub(super) fn encode_allocation_at_location(
        &self,
        location: mir::Location,
    ) -> SpannedEncodingResult<Vec<vir::Stmt>> {
        trace!("Encode ownership before {:?}", location);
    ...
    fn get_deeply_unreachable_places_at(
        &self,
        location: mir::Location,
    ) -> SpannedEncodingResult<FxHashSet<Place<'tcx>>> {
        ...
        if stmt_or_term.is_left() {
            ...
            Ok(unreachable
                .values()
                .next()
                .map(|s| s.get_deeply_unreachable_places())
                .unwrap_or_default())
        }
    }

    fn get_shallowly_unreachable_places_at(
        &self,
        location: mir::Location,
    ) -> SpannedEncodingResult<FxHashSet<Place<'tcx>>> {
        let stmt_or_term = self.mir.stmt_at(location);
        ...
        if stmt_or_term.is_left() {
            ...
            Ok(unreachable
                .values()
                .next()
                .map(|s| s.get_shallowly_unreachable_places())
                .unwrap_or_default())
        }
    }
}
