...
pub trait WithEncoder<'v, 'tcx: 'v> {
    ...
    fn is_pure_stable(
        ...
        caller_def_id: Option<DefId>,
        mut def_id: DefId,
        mut substs: ty::SubstsRef<'tcx>,
    ) -> bool {
        if let Some(actual_caller_def_id) = caller_def_id {
            (def_id, substs) =
                self.env()
                    .query
                    .resolve_method_call(actual_caller_def_id, def_id, substs);
        }
    ...
    fn is_ghost(&self, caller_def_id: Option<DefId>, mut def_id: DefId) -> bool {
        if let Some(actual_caller_def_id) = caller_def_id {
            (def_id, _) = self.env().query.resolve_method_call(
                actual_caller_def_id,
                def_id,
                self.env().query.identity_substs(def_id),
            );
        }
    ...
    fn encode_snapshot_domain(
        ...
        Ok(match kind {
            SnapshotKind::Memory => {
                box MemSnapshotDomain::encode(self.encoder(), ty)? as Box<dyn SnapshotBuilder>
            }
            SnapshotKind::Value => {
                box ValueSnapshotDomain::encode(self.encoder(), ty)? as Box<dyn SnapshotBuilder>
            }
        })
    }
}
