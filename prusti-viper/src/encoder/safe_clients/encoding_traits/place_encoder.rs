...
pub trait PlaceEncoder<'v, 'tcx: 'v>: WithLocalTy<'v, 'tcx> {
    ...
    fn encode_deref(&self, address: vir::Expr, ty: ty::Ty<'tcx>) -> EncodingResult<Option<SnapshotExpr>> {
    ...
    fn encode_place_projection_snapshot(
        &self, base_snapshot: SnapshotExpr, base_ty: PlaceTy<'tcx>,
            ...
            mir::ProjectionElem::Deref => {
                match *base_ty.ty.kind() {
                    ...
                    ty::TyKind::RawPtr(ty::TypeAndMut { ty: target_ty, .. }) => {
                        let kind = base_snapshot.kind();
                        let address = self.encode_snapshot_domain(kind, base_ty.ty)?
                        ...
                        SnapshotExpr::new(
                            AddressDomain::encode(self.encoder(), target_ty)?.deref_function()?
    ...
    fn encode_place_projection_address(
        &self, base_address: vir::Expr, base_ty: PlaceTy<'tcx>,
        projection: mir::PlaceElem<'tcx>,
    ) -> EncodingResult<Option<vir::Expr>> {
            ...
            mir::ProjectionElem::Deref => {
                match base_ty.ty.kind() {
                    &ty::TyKind::Ref(_, target_ty, _) => {
                        ...
                        let snapshot_domain = self.encode_snapshot_domain(place_snapshot.kind(), base_ty.ty)?;
                        snapshot_domain
                            .target_address_function()?
                    ...
                    ty::Adt(adt_def, substs) if adt_def.is_box() => {
                        ...
                        debug_assert!(place_snapshot.kind().is_memory());
                        let (ptr_snapshot, ptr_ty) = self.encode_box_content(place_snapshot, base_ty)?;
                        debug_assert!(ptr_snapshot.kind().is_memory());
                        let snapshot_domain = self.encode_snapshot_domain(ptr_snapshot.kind(), ptr_ty.ty)?;
                        snapshot_domain
                            .target_address_function()?
                            .apply1(ptr_snapshot)
                    }
    ...
    fn encode_operand_address(&self, operand: &mir::Operand<'tcx>) -> EncodingResult<Option<vir::Expr>> {
        match operand {
    ...
    fn encode_operand_snapshot(&self, operand: &mir::Operand<'tcx>) -> EncodingResult<SnapshotExpr> {
    ...
    fn encode_constant_snapshot(&self, constant: mir::Constant<'tcx>) -> EncodingResult<SnapshotExpr> {
    ...
    fn unify_typed_expressions(
        &self, exprs: Vec<SnapshotExpr>, types: &[ty::Ty<'tcx>],
    ...
    fn convert_to_value_snapshot(
        ...
        let frame = open_trace!("convert_to_value_snapshot", format!("{:?}: {ty:?}", expr.kind()));
    ...
    fn convert_to_memory_snapshot(
        ...
        let frame = open_trace!("convert_to_memory_snapshot", format!("{:?}: {ty:?}", expr.kind()));
        if expr.kind().is_memory() {
            Ok(expr.into_expr())
        } else if ty.is_unsafe_ptr() {
            // The conversion function doesn't always exist
            let expr_target_address = self.encode_snapshot_domain(expr.kind(), ty)?
                .target_address_function()?.apply1(expr);
            Ok(MemSnapshotDomain::encode(self.encoder(), ty)?
                .constructor_function()?.apply1(expr_target_address))
        } else {
            Ok(ValueSnapshotDomain::encode(self.encoder(), ty)?
    ...
    fn encode_box_content(&self, box_snapshot: SnapshotExpr, box_ty: PlaceTy<'tcx>) -> EncodingResult<(SnapshotExpr, PlaceTy<'tcx>)> {
        ...
        let nonnull_field = if let ty::TyKind::Adt(unique_adt_def, unique_substs) = unique_ty.ty.kind() {
        ...
        let ptr_field = if let ty::TyKind::Adt(nonnull_adt_def, nonnull_substs) = nonnull_ty.ty.kind() {
        ...
        let unique_snap = self.encode_place_projection_snapshot(
            box_snapshot,
            box_ty,
            unique_field
        )?;
        let nonnull_snap = self.encode_place_projection_snapshot(
            unique_snap,
            unique_ty,
            nonnull_field
        )?;
        let ptr_snap = self.encode_place_projection_snapshot(
            nonnull_snap,
            nonnull_ty,
            ptr_field
        )?;
