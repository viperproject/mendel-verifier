...
use crate::encoder::{
    high::builtin_functions::HighBuiltinFunctionEncoderInterface, mir::pure::PureEncodingContext,
    safe_clients::prelude::*,
};

/// Trait used to encode the body of a pure item (functions, triggers, predicates, assertions...).
pub(crate) trait PureEncoder<'v, 'tcx: 'v>:
    MirExprEncoder<'v, 'tcx> + CallEncoder<'v, 'tcx>
{
    fn pure_encoding_context(&self) -> PureEncodingContext;

    fn impl_encode_failing_assertion(
        &self,
        ...
        Ok(match self.pure_encoding_context() {
            PureEncodingContext::Code => {
                ...
                self.encode_requires_false_call(pos, domain_kind)
                    .with_span(span)?
            }
            PureEncodingContext::Assertion => {
                ...
                match domain_kind {
                    ...
                    BuiltinDomainKind::ValueSnapshot(ty) => {
                        debug_assert!(ty.is_bool());
                        ValueSnapshotDomain::encode(self.encoder(), ty)
                            .with_default_span(span)?
                            .constructor_function()
                            .with_default_span(span)?
                            .apply1(value_expr)
    ...
    fn encode_body(&self, context: GhostOrExec) -> SpannedEncodingResult<vir::Expr> {
        ...
        let encoded_body = self
            .encode_mir_expr_snapshot(&mir_expr, context)
    ...
    fn encode_requires_false_call(
        ...
        let (function_name, type_arguments) = self
            .encoder()
            .encode_builtin_function_use(BuiltinFunctionKind::Unreachable(domain_ty.clone()));
