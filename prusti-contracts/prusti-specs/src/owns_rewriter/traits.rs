...
pub fn rewrite_trait(item_trait: &syn::ItemTrait, owns_attr: &TokenStream) -> syn::Result<TokenStream> {
    let generated_struct = generate_new_struct(item_trait)?;

    let trait_impl = generated_struct.generate_impl(owns_attr)?;
...
fn generate_new_struct(
    ...
    let parsed_generics: Vec<TypeParam> = item_trait
        .generics
        .type_params()
        .cloned()
        .collect();
...
impl<'a> GeneratedStruct<'a> {
    fn generate_impl(&self, owns_attr: &TokenStream) -> syn::Result<syn::ItemImpl> {
        // Generate impl block
